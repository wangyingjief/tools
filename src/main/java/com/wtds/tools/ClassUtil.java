 ...
 * 并可通过Annotation(类注)来过滤，避免一些内部类的干扰.<br/>
 * 
 * @author joymting
 */
...
//		String pkg = "com.master.bean";
//		List list = getClassList(pkg, recursive, DBTable.class);
...
//		}
	 ...
	 * 获取所有class类
	 * @param pkgName 包名
	 * @param isRecursive 是否要遍历所有子包
	 * @param annotation 注解类型，传递null为获取所有
	 * @return
	 */
...
//				System.out.println("prefix:" + prefix +" pkgName:" + pkgName);
	...
	private static void addClassName(List<Class<?>> clazzList, String clazzName, Class<? extends Annotation> annotation) {
		if (clazzList != null && clazzName != null) {
			Class<?> clazz = null;
			try {
				if(clazzName.indexOf(".class") > 0)clazzName = clazzName.replace(".class", "");
