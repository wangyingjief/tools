...
public class Check {
	...
	public static boolean fcsCheck(byte[] content, byte checkSum, int start,
			int end) {
		byte tempSum = 0;
		while (start <= end) {
	...
	public static boolean dataCheck(byte[] data) {
		try {
			...
			if ((data.length > 21)
					&& (data[15+ Transfer.byteToShort(data, 13)+2-1] == (byte) 0xBA) ? true
					: false) {
				switch (data[11]) {
				case 0x01:
	...
	//文件移动
	public static void backupFile(File messageFile, File backupDirectory,
			String sonDirectory) {
